...
if( ! function_exists( 'aiop_pri' ) ) :
function aiop_pri( $data ) {
	echo '<pre>';
 ...
 */
if( ! function_exists( 'aiop_get_posts' ) ) :
function aiop_get_posts( $args = [], $show_heading = true, $show_cached = true ) {
	...
	// use cache
	if( true === $show_cached && ( $cached_posts = wp_cache_get( "aiop_{$_args['post_type']}", 'aiop' ) ) ) {
		$posts = $cached_posts;
	}
	...
	// don't use cache
		...
		wp_cache_add( "aiop_{$_args['post_type']}", $posts, 'aiop', 3600 );
	}

	$posts = $show_heading ? [ '' => sprintf( __( '- Choose a %s -', 'aiop' ), $_args['post_type'] ) ] + $posts : $posts;

	return apply_filters( 'aiop_get_posts', $posts, $_args );
}
endif;

if( ! function_exists( 'aiop_get_option' ) ) :
function aiop_get_option( $key, $section, $default = '' ) {
...
if( !function_exists( 'aiop_get_template' ) ) :
/**
 * Includes a template file resides in /views diretory
 *
 * It'll look into /all-in-one-page directory of your active theme
 * first. if not found, default template will be used.
 * can be overriden with all-in-one-page_template_override_dir hook
 *
 ...
 */
function aiop_get_template( $slug, $base = 'views', $args = null ) {

	// templates can be placed in this directory
	$override_template_dir = apply_filters( 'aiop_template_override_dir', get_stylesheet_directory() . '/all-in-one-page/', $slug, $base, $args );
	
	// default template directory
	$plugin_template_dir = dirname( AIOP ) . "/{$base}/";
	...
	elseif ( file_exists( $plugin_template_path ) ) {
		...
		return __( 'Template not found!', 'all-in-one-page' );
	}
}
endif;
 ...
 * @since 1.0
 */
if( !function_exists( 'aiop_action_link' ) ) :
function aiop_action_link( $plugin, $action = '' ) {
