...
class Survey extends Base {
	...
	public function __construct( $plugin ) {
		...
		$this->api_url 	= "{$this->server}/wp-json/wpplugines-lead/v1.0/";
		
		$this->notice_heading = '<h3>' . sprintf( __( 'Thanks for using \'<strong>%s</strong>\'', 'wpplugines' ), $this->name ) . '</h3>';
		$this->notice_message = '<p> ' . __( 'We want to know what type of sites use this plugin. Users\' satisfaction is our first priority and we\'re continuously working on it. This is why we need some information so that we can improve it even more.<br />Help us with your site URL and a few basic information. It doesn\'t include your password or any secret data. Would you like to help us?', 'wpplugines' ) . '</p>';
		$this->notice_button_text = __( 'Okay. Don\'t bother me again!', 'wpplugines' );
	...
	public function enqueue_scripts() {
		wp_enqueue_style( $this->slug . '-survey', plugins_url( 'assets/css/survey.css', AIOP ), [], $this->plugin['Version'] );
		wp_enqueue_script( $this->slug . '-survey', plugins_url( 'assets/js/survey.js', AIOP ), [ 'jquery' ], $this->plugin['Version'], true );
	}
    ...
    public function admin_notices() {
        ...
        ?>
        <div id="<?php echo $this->slug; ?>-survey-notice" class="wpplugines-survey-notice notice notice-success is-dismissible wpplugines-survey wpplugines-notice wpplugines-shadow" data-slug="<?php echo $this->slug; ?>">

        	<img class="wpplugines-survey-img" src="<?php echo plugins_url( 'assets/img/survey.png', AIOP ); ?>" />
            <div class="wpplugines-survey-content">
                <?php echo $this->get_heading(); ?>
                <?php echo $this->get_message(); ?>
            </div>
            <p class="wpplugines-survey-btn-wrapper">
                <button class="button button-primary button-hero wpplugines-survey-btn" data-participate="1">
